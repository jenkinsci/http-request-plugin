...
link:https://ci.jenkins.io/job/Plugins/job/http-request-plugin/job/master/[image:https://ci.jenkins.io/buildStatus/icon?job=Plugins%2Fhttp-request-plugin%2Fmaster[Build]]
...
If the request cannot reach the server (unknown host, refused connection, timeout),
the build fails with an error message that names the problem and the URL, instead
of reporting a made-up HTTP status code:

[source]
----
Fail: Host does not exist or could not be resolved: example.invalid while calling https://example.invalid/api
----
